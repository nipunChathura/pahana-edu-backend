    ...
    private void setPromotion(Long bookId, BigDecimal bookPrice, BookDetailsDto bookDetailsDto) {
        ...
        if (!activePromotionByBookNow.isEmpty()) {
            ...
            if (activePromotion.getPromotionType().equalsIgnoreCase(Constants.PROMOTION_FLAT_TYPE)) {
                bookDetailsDto.setMinusAMount(promotionPrice);
                bookDetailsDto.setPromotionBookPrice(BigDecimal.valueOf( Math.abs(bookPrice.doubleValue()-promotionPrice.doubleValue())));
            } else if (activePromotion.getPromotionType().equalsIgnoreCase(Constants.PROMOTION_PERCENTAGE_TYPE)) {
                double promotion = (bookPrice.doubleValue() * (promotionPrice.doubleValue()/100));
                bookDetailsDto.setMinusAMount(BigDecimal.valueOf(promotion));
                bookDetailsDto.setPromotionBookPrice(BigDecimal.valueOf(bookPrice.doubleValue() - promotion));
            }
        }
    }
}
